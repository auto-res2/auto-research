...
def run_experiment(config_path, test_run=False):
    ...
    print("\n" + "="*70)
    print("TwiST-Distill Experiment Configuration".center(70))
    print("="*70)
    ...
    print(f"Training epochs: {config['training']['num_epochs']} | Learning rate: {config['training']['learning_rate']}")
    print(f"Optimizer: {config['training']['optimizer']} | Scheduler: {config['training']['scheduler']}")
    print(f"Weight decay: {config['training']['weight_decay']}")
    print(f"Consistency weight: {config['training']['consistency_weight']} | Noise std: {config['training']['noise_std']}")
    print(f"Evaluation metrics: {', '.join(config['evaluation']['metrics'])}")
    print(f"Noise levels for evaluation: {config['evaluation']['noise_levels']}")
    print(f"Test run: {config['experiment']['test_run']} | Verbose: {config['experiment']['verbose']}")
    print(f"Random seed: {config['experiment']['seed']}")
    print(f"CUDA memory allocation config: expandable_segments:True")
    print("="*70 + "\n")
